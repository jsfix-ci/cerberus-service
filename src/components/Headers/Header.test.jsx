...
describe('Header', () => {
  ...
  describe('AirpaxTasks', () => {
    ...
    it('should render the Tasks wording as a URL when on roro task list page, on the header', () => {
      extendedRouterMock.useLocation = jest.fn().mockReturnValue({ pathname: PATHS.RORO });
      render(<Header />);
      expect(screen.getByText('Tasks')).toBeInTheDocument();
    });

    it('should render the Tasks wording as a URL when on airpax task list page, on the header', () => {
      extendedRouterMock.useLocation = jest.fn().mockReturnValue({ pathname: PATHS.AIRPAX });
      render(<Header />);
      expect(screen.getByText('Tasks')).toBeInTheDocument();
    });

    it('should render the Tasks wording as a URL when on RoRo V2 task list page, on the header', () => {
      extendedRouterMock.useLocation = jest.fn().mockReturnValue({ pathname: PATHS.RORO_V2 });
      render(<Header />);
      expect(screen.getByText('Tasks')).toBeInTheDocument();
    });
  });
