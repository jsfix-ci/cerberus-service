...
describe('VoyageSection', () => {
  it('should render the voyage section for an airpax task', () => {
    const tree = renderer.create(
      <VoyageSection view={VIEW.AIRPAX} targetTask={mockAirpaxTargetTask} refDataAirlineCodes={refDataAirlineCodes} />,
    ).toJSON();
  ...
  it('should render the voyage section for a RoRo v2 accompanied task', () => {
    const tree = renderer.create(
      <VoyageSection view={VIEW.RORO} targetTask={mockRoRoTargetTask} refDataAirlineCodes={refDataAirlineCodes} />,
    ).toJSON();
    expect(tree).toMatchSnapshot();
  });
  ...
  it('should render the voyage section for a RoRo v2 foot passenger task', () => {
    const tree = renderer.create(
      <VoyageSection view={VIEW.RORO} targetTask={mockRoRoFootPassengerTask} refDataAirlineCodes={refDataAirlineCodes} />,
    ).toJSON();
    expect(tree).toMatchSnapshot();
  });
});
